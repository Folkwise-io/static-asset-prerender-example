...
let logoImg: Image, iconImg: Image;
...
loadImage(path.join(__dirname, "../", "assets", "logo.png")).then((image) => {
	logoImg = image;
});

loadImage(path.join(__dirname, "../", "assets", "bean.png")).then((image) => {
	iconImg = image;
});
...
export default class ImageBuilder {
	...
	logo({ x, y }: Coords) {
		this.context.drawImage(logoImg, x, y, 410, 115);
		return this.reset();
	}
